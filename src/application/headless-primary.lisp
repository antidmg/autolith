...
(defclass primary-job-request nil
  ...
  (:documentation "One validated headless primary-agent turn request."))
...
(-> primary-job--evidence
    ((option application) (option (integer 1)) list (integer 0)
     &key (:result (option provider-result)))
    list)
(defun primary-job--evidence
    (application start-sequence events journal-start-count &key result)
  "Return durable conversation, observer, and mutation evidence for one turn."
  (let* ((conversation
           (and application
...
(defun primary-job-execute-with-application
  ...
  "Execute REQUEST through the primary application agent with self tools intact."
  (let ((application nil)
        (events nil)
    ...
    (labels ((observe-status (status details)
               "Retain one portable status event without imposing an output ceiling."
             ...
             (evidence ()
               ...
               (primary-job--evidence
                application start-sequence events journal-start-count
                :result result))
           ...
           (handler-case
               (progn
                 ...
                 (setf journal-start-count
                       (length (mutation-journal-read-records
                                (application-configuration application))))
...
(defun primary-job-result-envelope
  ...
  "Return one version-one terminal primary-job result envelope."
  (append
