...
Set =AUTOLITH_REALM_MODEL_COMMAND= to an absolute =ethereal= executable to
route Chat Completions turns through a local Realm daemon instead of connecting
directly. The bridge also requires =AUTOLITH_REALM_ID=,
=AUTOLITH_REALM_MODEL_ARTIFACT=, and =AUTOLITH_REALM_ENGINE_GENERATION=.
=AUTOLITH_REALM_MODEL_TIMEOUT_MS= optionally sets the per-turn timeout and
defaults to 300000. Realm receives the complete JSON request, calls only the
provider's configured loopback HTTP endpoint, retains the response body and raw
headers as artifacts, and returns the retained SSE body to Autolith. Provider
credentials and configured custom headers are not forwarded through this local
bridge. Model discovery remains on the provider's normal path.
...
=run-primary-job= is the explicit non-interactive boundary for the primary
agent. Unlike =run-job=, it preserves the primary registry, including =self.*=
when Autolith was not launched with =--immutable=. It does not make =self.*=
child-safe or change child-role policy.

#+BEGIN_SRC sh
autolith --permissions auto run-primary-job \
  --input /path/to/primary-job.sexp \
  --output /path/to/primary-result.sexp
#+END_SRC

The version-two input names its durable conversation, fences the exact next
sequence and loaded generation, and declares every model and tool budget:

#+BEGIN_SRC lisp
(:autolith-primary-job
 :version 2
 :id "realm-effect-id"
 :conversation-id "realm-continuation-id"
 :expected-next-sequence 1
 :expected-generation "retained-generation-id"
 :prompt "Inspect the failure and improve the active image."
 :reasoning-effort "low"
 :maximum-output-tokens 1024
 :maximum-provider-requests 6
 :tool-allowlist ("resource.read" "self.redefine" "self.diff"
                  "self.exercise" "self.commit" "self.discard")
 :timeout-seconds 120)
#+END_SRC

Use =NIL= for =:expected-generation= only when clean tracked source is required.
The command fails before persisting a user message or making a provider request
when recovery fallback is active, the loaded generation differs, an allowed
tool is unavailable, the reasoning selection is invalid, or adaptive guidance
cannot render. The exact allowlist is advertised and enforced for fabricated
calls as well as normal tool selection.

A mismatched =:expected-next-sequence= fails before another user message or
provider request, so retrying an uncertain completed turn cannot duplicate it
silently. Each provider request receives =:maximum-output-tokens=, and the
complete turn cannot exceed =:maximum-provider-requests=. The atomic
=(:autolith-primary-job-result :version 2 ...)= artifact reports the
conversation cursor, final response, loaded-generation evidence, applied
limits, exact tool allowlist, portable observer events, aggregate usage, and
the mutation-journal records appended during the job. Continue with the
returned =:next-sequence=. The in-process deadline interrupts the active turn;
the calling supervisor still owns the hard process-lifetime bound.
