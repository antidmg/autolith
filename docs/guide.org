...
The input names its durable conversation and fences the exact next sequence:
...
The command opens or creates that exact conversation. A mismatched
=:expected-next-sequence= fails before another user message or provider request,
so retrying an uncertain completed turn cannot duplicate it silently. The
atomic =(:autolith-primary-job-result :version 1 ...)= artifact reports the
conversation cursor, final response, portable observer events, aggregate usage,
and the exact mutation-journal records appended during the job. Continue with
the returned =:next-sequence=. The in-process deadline interrupts the active
turn; the calling supervisor still owns the hard process-lifetime bound.
